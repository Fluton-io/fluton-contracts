...
NOTE: For resetting voyager follow commands in bellow:
```
psql 'postgres://<USERNAME>:<PASSWORD>/<DATABASE_NAME>'

delete from queue;
delete from optimize;
```

Note: For dependency errors:
```
nix develop
```
...
```
nix run .\#cosmwasm-scripts.union-devnet.deploy -- --relayers-admin union1jk9psyhvgkrt2cumz8eytll2244m2nnz4yt2g2 --rate-limit-admin union1jk9psyhvgkrt2cumz8eytll2244m2nnz4yt2g2 --relayers union1jk9psyhvgkrt2cumz8eytll2244m2nnz4yt2g2 --rate-limit-operators union1jk9psyhvgkrt2cumz8eytll2244m2nnz4yt2g2
```
...
forge-1     | ==========================
forge-1     |
forge-1     | ONCHAIN EXECUTION COMPLETE & SUCCESSFUL.
forge-1     |
forge-1     | Transactions saved to: /evm/broadcast/Deploy.s.sol/32382/run-latest.json
forge-1     |
forge-1     | Sensitive values saved to: /evm/cache/Deploy.s.sol/32382/run-latest.json
forge-1     |
forge-1 exited with code 0
```

Additional step
```
cast send 0xed2af2aD7FE0D92011b26A2e5D1B4dC7D12A47C5 "function registerClient(string calldata,address)" cometbls 0xc4f27a952faBa4174ce0Ee6D9d0c6F4c41524d49 --private-key 4e9444a6efd6d42725a250b650a781da2737ea308c839eaccb0f7f3dbd2fea77
```
...
5. Start voyager queue (PSQL db) (We don't need that!)
